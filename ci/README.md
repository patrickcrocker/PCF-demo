...
This project provides a [Concourse](https://concourse-ci.org/) pipeline based on
a fairly common real-world use case of pushing tested, built artifacts into S3 buckets.
...
- Concourse (see [Concourse Quick Start](https://concourse-ci.org/install.html#quickstart) to run locally)
- [Amazon S3](https://aws.amazon.com/s3) compatible storage (use the real thing or try [Minio](https://www.minio.io/))
- Cloud Foundry (see [Pivotal Web Services](http://run.pivotal.io/) for a free 60
day trial, or [PCF Dev](https://pivotal.io/pcf-dev) to run locally)

## Login to Concourse

Login to your Concourse server by giving it an alias.
...
To download the Fly CLI, navigate to the main page where there are links to binaries
...
```
$ cd PCF-demo
$ fly -t main execute -c ci/tasks/unit.yml -i pcfdemo=.
```
...
```
$ fly -t main execute -c ci/tasks/build.yml -i pcfdemo=. -i version=target/version
```
...
for configuring our pipeline.

```
$ cp pcfdemo-properties-sample.yml pcfdemo-properties.yml
```
...
```
$ fly -t main set-pipeline -p pcfdemo -c ci/pipeline.yml -l ci/pcfdemo-properties.yml
```

If you refresh the Concourse web ui, you should now see the `pcfdemo` pipeline. By
...
```
$ fly -t main unpause-pipeline -p pcfdemo
```
...
```
$ fly -t main trigger-job -j pcfdemo/ship-it
```
